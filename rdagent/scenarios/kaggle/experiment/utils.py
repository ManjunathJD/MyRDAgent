...
def python_files_to_notebook(competition: str, py_dir: str):
    ...
    for fea_file in fea_files:
        content = fea_file.read_text()
        content = content.replace("feature_engineering_cls", f"{fea_file.stem}_cls").strip()
        content += "()\n"
        fea_pys[f"{fea_file.stem}_cls"] = content

    model_files = list(py_dir.glob("model/model_*.py"))
    ...
    for select_file in select_files:
        ...
        content = content.replace("def select(", f"def {select_file.stem}(")
        select_pys[f"{select_file.stem}"] = content
    ...
    train_py = (
        train_py.replace('for f in DIRNAME.glob("model/model*.py"):', f"for mc in {model_cls_list_str}:").replace(
            "m = import_module_from_path(f.stem, f)", "m = mc()"
        ...
        .replace(
            ...
            'select_m = eval(mc.__name__.replace("model_", "select_"))',
        )
